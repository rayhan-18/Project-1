...
async function createOrder(orderData) {
  ...
  try {
    ...
    const [orderResult] = await connection.query(
      `INSERT INTO orders 
      (customer_name, customer_phone, customer_address, shipping_method, shipping_cost, payment_method, subtotal, tax, total, status)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        ...
        orderData.tax,
        orderData.total,
        'pending' // Default status saat order dibuat
      ]
    );
